...
{ /* pt=e*pa; */
...
#ifdef MR_STATIC
    char mem[MR_ECP_RESERVE(MR_STR_SZ)];  
#else
    char *mem;
#endif
    ...
    if (mr_mip->base==mr_mip->base2)
    {
...
#ifdef  MR_STATIC
        memset(mem,0,MR_ECP_RESERVE(MR_STR_SZ));
#else
        mem=(char *)ecp_memalloc(_MIPP_ MR_STR_SZ);
#endif
        ...
        epoint_copy(pt,table[MR_STR_SZ-1]);
        ecurve_double(_MIPP_ table[MR_STR_SZ-1]);
     /*   epoint_norm(_MIPP_ table[MR_STR_SZ-1]); */

        for (i=1;i<MR_STR_SZ-1;i++)
        { /* precomputation */
            epoint_copy(table[i-1],table[i]);
            ecurve_add(_MIPP_ table[MR_STR_SZ-1],table[i]);
        }
        ecurve_add(_MIPP_ table[MR_STR_SZ-2],table[MR_STR_SZ-1]);
