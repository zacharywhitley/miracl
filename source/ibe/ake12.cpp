/*
   ...
   Compile as 
   cl /O2 /GX /DZZNS=8 ake12.cpp zzn12.cpp zzn6a.cpp ecn2.cpp zzn2.cpp big.cpp zzn.cpp ecn.cpp miracl.lib
   using COMBA build
...
BOOL fast_tate_pairing(ECn& P,ZZn2& Qx,ZZn2& Qy,Big &x,ZZn12 &X,ZZn6& res)
{ 
    ECn A;
    int i,nb;
    Big n,p,q,t;
...
// Also does not need precomputation (x is sparse)
    ...
    r*=a;
#ifdef MR_COUNT_OPS
printf("After final exp fpc= %d fpa= %d\n",fpc,fpa);
#endif
    res= real(r);
    return TRUE;
}
...
BOOL ecap(ECn& P,ECn2& Q,Big& x,ZZn12 &X,ZZn6& r)
{
    ...
    Ok=fast_tate_pairing(PP,Qx,Qy,x,X,r);
