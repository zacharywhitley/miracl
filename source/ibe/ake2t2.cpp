/*
   ...
   Compile as 
   cl /O2 /GX /DZZNS=16 ake2t2.cpp ecn2.cpp zzn2.cpp big.cpp zzn.cpp ecn.cpp miracl.lib
   using COMBA build
...
// Evaluate line from A - lam is line slope
    ptr=A.add(B);
    if (ptr==NULL)    return 1; //vertical(P,Qx);
...
// Alice, Bob are points of order q
    ...
    if (!tate(cf1*Trace(sA),Server,q,res)) cout << "Trouble" << endl; 
    ...
    if (!tate(cf1*Trace(Alice),sS,q,res))  cout << "Trouble" << endl;
    ...
    if (!tate(cf1*Trace(sB),Server,q,res)) cout << "Trouble" << endl;
    if (powl(res,q)!=(ZZn)1)
    {
        cout << "Wrong group order - aborting" << endl;
        exit(0);
    ...
    if (!tate(cf1*Trace(Bob),sS,q,res)) cout << "Trouble" << endl;
    sp=powl(res,s);
    ...
    if (!tate(cf1*Trace(sB),Alice,q,res)) cout << "Trouble" << endl;
    if (powl(res,q)!=(ZZn)1)
    {
        cout << "Wrong group order - aborting" << endl;
        exit(0);
    ...
    if (!tate(cf1*Trace(Bob),sA,q,res)) cout << "Trouble" << endl;
    ap=powl(res,a);
